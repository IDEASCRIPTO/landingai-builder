...
function normalizeCopy(raw){
  ...
  function toObj(v, tipo){
    ...
    switch(tipo){
      ...
      case 'popup_social': return {messages:lines.length?lines:['🛒 Alguien acaba de ordenar'],cta:''};
...
.vid-carousel-item{flex:0 0 180px;height:320px;scroll-snap-align:center;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.12)}
.vid-carousel-item iframe{width:100%;height:100%;border:none;display:block}
.vid-carousel-ph{width:100%;height:100%;background:#111;display:flex;align-items:center;justify-content:center;font-size:3rem;opacity:.5}
@media(max-width:680px){.vid-carousel-item{flex:0 0 60vw;height:106.7vw}}
...
// POPUP — Pill rotante con mensajes de prueba social
if (secSet.has('popup_social')) {
  const msgs = Array.isArray(pop.messages) && pop.messages.length > 0
    ? pop.messages
    : ['🛒 Alguien acaba de ordenar este producto'];
  const firstMsg = esc(msgs[0] || '');
  const msgsJson = JSON.stringify(msgs.map(m => String(m || '')));

  secs += `<div class="ps ps1" id="pop"><div class="ps1-av">🛒</div><div class="ps1-b"><span id="p1msg">${firstMsg}</span></div></div>`;
  popScript = `(function(){var p=document.getElementById('pop'),ms=${msgsJson},i=0;function show(){if(!p)return;document.getElementById('p1msg').textContent=ms[i%ms.length];p.classList.add('vis');setTimeout(function(){p.classList.remove('vis');setTimeout(function(){i++;show();},1200);},6000);}setTimeout(show,3500);})();`;
}
