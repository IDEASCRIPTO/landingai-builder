...
(function(){
  ...
  bar.addEventListener('click',function(){
    ...
    if ($meta_pixel || $tiktok_pixel || $ga_id) {
        ...
        if ($meta_pixel) {
            $pixel_head .= "<script>!function(f,b,e,v,n,t,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';n.queue=[];t=b.createElement(e);t.async=!0;t.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(t,s)}(window,document,'script','https://connect.facebook.net/en_US/fbevents.js');var _eid=(function(){var k='_fbeid',v=localStorage.getItem(k);if(!v){v=Math.random().toString(36).slice(2,10)+Date.now().toString(36);localStorage.setItem(k,v);}return v;})();fbq('init','{$meta_pixel}',{external_id:_eid});fbq('track','PageView');fbq('track','ViewContent',{content_ids:['{$prod_id}'],content_type:'product',content_name:'{$prod_name}',value:{$prod_price},currency:'{$prod_currency}'});</script>\n<noscript><img height='1' width='1' style='display:none' src='https://www.facebook.com/tr?id={$meta_pixel}&ev=PageView&noscript=1'/></noscript>\n";
        }
        if ($tiktok_pixel) {
        ...
        $pixel_events .= "  }\n";
        $pixel_events .= "  async function _h256(s){const b=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(s));return Array.from(new Uint8Array(b)).map(x=>x.toString(16).padStart(2,'0')).join('');}\n";
        $pixel_events .= "  document.addEventListener('submit',async function(e){if(!_mp||typeof fbq==='undefined')return;var f=e.target,ei=f.querySelector('input[type=email],[name*=email],[name*=correo]'),pi=f.querySelector('input[type=tel],[name*=phone],[name*=tel],[name*=whatsapp]'),ud={external_id:typeof _eid!=='undefined'?_eid:''};if(ei&&ei.value.trim())ud.em=await _h256(ei.value.trim().toLowerCase());if(pi&&pi.value.trim())ud.ph=await _h256(pi.value.replace(/\\D/g,''));if(ud.em||ud.ph){fbq('init',_mp,ud);fbq('trackSingle',_mp,'Lead',{content_name:_pn,value:_pv,currency:_pc});}if(_tp&&typeof ttq!=='undefined'){var _tid={};if(ud.em)_tid.email=ud.em;if(ud.ph)_tid.phone_number=ud.ph;if(Object.keys(_tid).length)ttq.identify(_tid);ttq.track('SubmitForm',{content_id:_pi,content_type:'product'});}if(_ga&&typeof gtag==='function'){if(ud.em)gtag('config',_ga,{user_id:ud.em});gtag('event','generate_lead',{currency:_pc,value:_pv});}});\n";
